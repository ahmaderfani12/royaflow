...
// Using one from FluidSimulater (TODO: Make it clean)
...
public class FluidSimulater3D
{
    ...
    public CommandBuffer GetCommandBuffer() => sim_command_buffer;

    public RenderTexture visulasation_texture;
    ...
    public FluidSimulater3D()                       // Default Constructor
    {
    ...
    public FluidSimulater3D(FluidSimulater other)   // Copy Constructor
    {
    ...
    public void Visualiuse(ComputeBuffer buffer_to_visualize, bool overrideOnCamera = true)
    {
        if (!IsValid()) return;
